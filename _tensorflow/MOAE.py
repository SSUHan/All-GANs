...
class MOAE(BASE):
    ...
    def encoder(self, x, is_training=True, reuse=False):
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            net = lrelu(conv2d(x, 128, kernel_hw=(3, 3), stride_hw=(2, 2), name='en_conv1'))
            net = lrelu(batch_norm(conv2d(net, 256, kernel_hw=(3, 3), stride_hw=(2, 2), name='en_conv2'),
                                   is_training=is_training, scope='en_bn2'))
            net = lrelu(batch_norm(conv2d(net, 256, kernel_hw=(3, 3), stride_hw=(2, 2), name='en_conv3'),
                                   is_training=is_training, scope='en_bn3'))
            net = lrelu(batch_norm(conv2d(net, 256, kernel_hw=(3, 3), stride_hw=(2, 2), name='en_conv4'),
